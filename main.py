...
def detect(save_img=False): 
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det): #当使用webcam时，len(det) = 0，此处不工作(why?)
                ...
                #thread.join()
                #if(accel_frame % fps_set == 0):
                # Write results on every frame 将结果写至每一帧
                            ...
                            if (0< x_0 < 1280):
                                ...
                                if (dis_avg!=0):## Add bbox to image
                                    ...
                                    for track in tracks:
                                        ...
                                        cv2.putText(im0, str(indexID), (int(bbox[0]), int(bbox[1] - 10)), 0, 1, colors[int(cls)], 1)
